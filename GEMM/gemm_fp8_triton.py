"""Triton FP8 GEMM with PyTorch comparison and benchmarking utilities."""
...
@triton.jit
def gemm_fp8_kernel(a_ptr, b_ptr, c_ptr,
                    M: tl.constexpr,
                    N: tl.constexpr,
                    K: tl.constexpr,
    ...
    if tma_path:
        # Use tensor descriptors so descriptor.load issues TMA-backed transfers when available.
    ...
    if tma_path:
        # Store the full block via TMA when the tile is fully inside bounds.
...
def launch_triton_gemm(a: torch.Tensor, b: torch.Tensor, out: torch.Tensor, use_tma: bool = True) -> None:
    """Launch the Triton fp8->fp32 GEMM kernel."""
    ...
    if a.dtype != FP8_DTYPE or b.dtype != FP8_DTYPE:
        raise ValueError("Input tensors must be FP8 for this GEMM example")
...
def benchmark_torch_matmul(
    ...
    """Benchmark torch.matmul on fp32 copies of the fp8 inputs."""
    def op():
...
def benchmark_triton_matmul(
    ...
    """Benchmark the Triton GEMM kernel."""
    def op():
...
def test():
    ...
    print(f"gemm_fp8_triton relative error: {error:.3e}")
