...
# Author: Liu Yiyang
# Date:   2026-02-01
# Purpose: Triton implementation for Flash GQA backward kernel.
# ==============================================================================
...
@triton.jit
def flash_gqa_backward_bf16_kernel(
    q_ptr,
    k_ptr,
    v_ptr,
    o_ptr,
    ...
    """
    Prototype Flash GQA backward kernel (bf16 inputs, fp32 accumulation).
    ...
    Inputs:
        ...
        rowwise_dot_o_grad_o: pointer to float32 with shape [batch, q_heads, seqlen_q]
    ...
    k_mask = (pid_kv * block_kv + tl.arange(0, block_kv))[:, None] < seqlen_kv
    ...
    for q_start in tl.range(seq_len_q_start, seqlen_q, block_q_sub):
        ...
        q_ptrs = q_ptr + pid_b * q_heads * seqlen_q * head_dim_qk + \
                 ...
                 (q_start + tl.arange(0, block_q_sub))[:, None] * head_dim_qk + d_offsets_qk[None, :]
        q_mask = (q_start + tl.arange(0, block_q_sub))[:, None] < seqlen_q
        q_block = tl.load(q_ptrs, mask=q_mask & dim_mask_qk[None, :], other=0.0)

        # load lse (block_q_sub,)
        lse_ptrs = lse_ptr + pid_b * q_heads * seqlen_q + \
                    q_head_idx * seqlen_q + \
                    (q_start + tl.arange(0, block_q_sub))
        lse_block = tl.load(lse_ptrs, mask=(q_start + tl.arange(0, block_q_sub)) < seqlen_q, other=0.0)
        ...
        if causal and q_start < pid_kv * block_kv + block_kv - 1:
            attn_mask = (q_start + tl.arange(0, block_q_sub))[:, None] >= (pid_kv * block_kv + tl.arange(0, block_kv))[None, :]
        ...
        grad_o_ptrs = grad_o_ptr + pid_b * q_heads * seqlen_q * head_dim_v + \
                       ...
                       (q_start + tl.arange(0, block_q_sub))[:, None] * head_dim_v + d_offsets_v[None, :]
        ...
        rowwise_dot_o_grad_o_ptrs = rowwise_dot_o_grad_o_ptr + pid_b * q_heads * seqlen_q + \
                                    q_head_idx * seqlen_q + \
                                    (q_start + tl.arange(0, block_q_sub))
        rowwise_dot_o_grad_o_block = tl.load(rowwise_dot_o_grad_o_ptrs, mask=(q_start + tl.arange(0, block_q_sub)) < seqlen_q, other=0.0)
...
def launch_flash_gqa_backward_bf16_kernel(
    q,
    k,
    v,
    o,
    ...
    """
    Launch the prototype Flash GQA backward kernel.
    ...
    Inputs:
        ...
        block_q: int32 scalar, query sub-block per flash GQA iteration
    ...
    if (
        ...
        raise ValueError("Flash GQA backward expects bf16 inputs and grad_o")
    if lse.dtype != torch.float32:
        raise ValueError("Flash GQA backward expects lse to be float32")
    if q.dim() not in (3, 4):
    ...
    flash_gqa_backward_bf16_kernel[grid](
        q_batched,
        k_batched,
        v_batched,
        o_batched,
