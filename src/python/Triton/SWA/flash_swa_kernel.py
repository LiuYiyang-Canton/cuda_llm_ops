...
# Author: Liu Yiyang
# Date:   2026-02-03
# Purpose: Triton implementation for Flash SWA forward kernel.
# ==============================================================================
...
@triton.jit
def flash_swa_forward_bf16_kernel(
    q_ptr,
    k_ptr,
    v_ptr,
    o_ptr,
    ...
    """
    Prototype Flash SWA forward kernel (bf16 inputs, fp32 accumulation).

    Main feature:
        Applies sliding-window attention with a sink term in the softmax denominator.
    ...
    Inputs:
        ...
        sink_value_ptr: pointer to float32 with shape [q_heads], sink term per query head
    ...
    window_start = q_offsets - (window_size - 1)
    ...
    # flash loop over kv tokens (clamp causal range and window start)
    seq_kv_end = min((pid_q + 1) * block_q, seqlen_kv)
    kv_start_min = pid_q * block_q - (window_size - 1)
    kv_start_min = tl.maximum(kv_start_min, 0)
    kv_offsets = kv_start_min + tl.arange(0, block_kv_sub)
    k_block_ptr = tl.make_block_ptr(
        base=k_ptr + k_base,
        shape=(seqlen_kv, head_dim_qk),
        strides=(head_dim_qk, 1),
        offsets=(kv_start_min, 0),
        block_shape=(block_kv_sub, block_dim_qk),
        order=(1, 0),
    )
    v_block_ptr = tl.make_block_ptr(
        ...
        offsets=(kv_start_min, 0),
        block_shape=(block_kv_sub, block_dim_v),
        order=(1, 0),
    )
    for kv_start in range(kv_start_min, seq_kv_end, block_kv_sub):
        ...
        window_mask = kv_offsets[None, :] >= window_start[:, None]
        ...
        p = tl.exp2((s - new_rowmax[:, None]) * 1.4426950408889634)
        p_bf16 = tl.cast(p, tl.bfloat16)
        alpha = tl.exp2((rowmax - new_rowmax) * 1.4426950408889634)
        o_head = tl.dot(p_bf16, v_block, acc=o_head * alpha[:, None], out_dtype=tl.float32)

        rowmax = new_rowmax
        rowsum = tl.sum(p, axis=1) + rowsum * alpha
        kv_offsets += block_kv_sub
        k_block_ptr = tl.advance(k_block_ptr, (block_kv_sub, 0))
        v_block_ptr = tl.advance(v_block_ptr, (block_kv_sub, 0))

    sink_value_block = tl.load(sink_value_ptr + head_idx, mask=head_idx < q_heads, other=0.0).to(tl.float32)
    sink_scaled = sink_value_block * tl.exp2(-rowmax * 1.4426950408889634)
    denom = rowsum + sink_scaled
    has_visible_kv = rowsum > 0
    safe_sink = tl.maximum(sink_value_block, 1.0e-9)
    o_head = tl.where(has_visible_kv[:, None], o_head / denom[:, None], 0.0)
    lse_head = tl.where(has_visible_kv, rowmax + tl.log(denom), tl.log(safe_sink))
...
def flash_swa_forward_grid(meta, q_heads, batch, seqlen_q, num_kv_heads):
    """
    Compute the grid for the Flash SWA forward kernel.
...
def launch_flash_swa_forward_bf16_kernel(
    q,
    k,
    v,
    scale=None,
    ...
    """
    Launch the prototype Flash SWA forward kernel.

    Main feature:
        Validates inputs, allocates outputs, and dispatches Triton with sliding-window attention.
    ...
    Inputs:
        ...
        block_kv_sub: int32 scalar, kv sub-block per flash SWA iteration
    ...
    if q.dtype != torch.bfloat16 or k.dtype != torch.bfloat16 or v.dtype != torch.bfloat16:
        raise ValueError("Flash SWA forward expects bf16 inputs")
    if q.dim() not in (3, 4):
    ...
    group_size = q_heads // num_kv_heads
    if group_size >= 16 and group_size % 16 == 0:
        block_g = 16
    elif group_size >= 8 and group_size % 8 == 0:
        block_g = 8
    elif group_size >= 4 and group_size % 4 == 0:
        block_g = 4
    elif group_size >= 2 and group_size % 2 == 0:
        block_g = 2
    else:
        block_g = 1
    ...
    block_dim_qk = head_dim_qk
    block_dim_v = head_dim_v
    if out is None:
    ...
    if sink_value.dim() != 1:
        raise ValueError("sink_value must be a 1D tensor with shape [q_heads]")
    if sink_value.numel() != q_heads:
        raise ValueError(f"sink_value must have {q_heads} elements")
    if sink_value.device != q_batched.device:
        raise ValueError("sink_value must be on the same device as Q")
    sink_value_batched = sink_value.contiguous()
    if sink_value_batched.dtype != torch.float32:
        sink_value_batched = sink_value_batched.to(torch.float32)

    launch_num_stages = num_stages
    if block_dim_qk > 128 or block_dim_v > 128:
        launch_num_stages = min(num_stages, 2)

    flash_swa_forward_bf16_kernel[grid](
        q_batched,
        k_batched,
        v_batched,
        out_batched,
        ...
        sink_value_ptr=sink_value_batched,
        num_warps=num_warps,
