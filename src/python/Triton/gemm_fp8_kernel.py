...
# Author: Liu Yiyang
# Date:   2026-01-30
# Purpose: Triton FP8 GEMM kernel and launch wrapper.
# ==============================================================================
"""Triton FP8 GEMM kernel and launch wrapper."""
...
def gemm_fp8_kernel(
    ...
    """
    Compute C = A @ B^T for FP8 inputs with FP32 accumulation.

    Main feature:
        Uses TMA when tiles are fully in-bounds, otherwise masked loads/stores.

    Inputs:
        a_ptr: pointer to fp8 with shape [m, k]
        b_ptr: pointer to fp8 with shape [n, k]
...
def launch_gemm_fp8_kernel(a, b, out, use_tma=True):
    """
    Launch the Triton FP8 GEMM kernel.
    ...
    if a.dim() != 2 or b.dim() != 2:
        raise ValueError("launch_gemm_fp8_kernel expects 2D matrices")
    ...
    if a.dtype != fp8_dtype or b.dtype != fp8_dtype:
        raise ValueError("Input tensors must be FP8 for this GEMM")
    ...
    if not (a.is_contiguous() and b.is_contiguous() and out.is_contiguous()):
        raise ValueError("launch_gemm_fp8_kernel expects contiguous tensors")

    m, k = a.shape
    n, _ = b.shape
    use_tma_flag = should_use_tma(use_tma, a.device)

    grid = functools.partial(gemm_fp8_grid, m=m, n=n)
    gemm_fp8_kernel[grid](a, b, out, m, n, k, use_tma=use_tma_flag)
