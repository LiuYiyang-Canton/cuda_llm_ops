...
NUM_GROUPS = NUM_QUERY_HEADS // GROUP_SIZE
HEAD_DIM = 128
...
def infer_flash_gqa_bf16_kernel(
    ...
    for g in range(0, BLOCK_G):
        ...
        for start_kv in range(0, seqlen_kv, BLOCK_KV):
            ...
            k_ptrs = k_ptr + (k_base[:, None] + kv_offsets_2d[:, None] + d_offsets[None, :])
            v_ptrs = v_ptr + (k_base[:, None] + kv_offsets_2d[:, None] + d_offsets[None, :])
...
def launch_flash_gqa(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, out: torch.Tensor | None = None) -> torch.Tensor:
    ...
    def grid(meta):
        return (
            triton.cdiv(NUM_GROUPS, meta["BLOCK_G"]),  # query head tiles
            batch,  # batch tiles
        )
...
def test(run_kernel: bool = True, warmup: int = 100, rep: int = 100):
    ...
    # Decoding: small query length, full KV context
    batch = 60
    seqlen_q = SEQLEN_Q
    seqlen_kv = SEQLEN_KV
    ...
    q = torch.rand((batch, q_heads, seqlen_q, HEAD_DIM), device=device, dtype=torch.bfloat16)
    k = torch.rand((batch, kv_heads, seqlen_kv, HEAD_DIM), device=device, dtype=torch.bfloat16)
    v = torch.rand((batch, kv_heads, seqlen_kv, HEAD_DIM), device=device, dtype=torch.bfloat16)
    out = torch.empty_like(q)
