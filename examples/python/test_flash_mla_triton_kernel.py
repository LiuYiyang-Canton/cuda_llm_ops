...
# Author: Liu Yiyang
# Date:   2026-01-31
# Purpose: Accuracy tests for Flash MLA Triton kernel (Training / Inference Prefill).
# ==============================================================================
...
def flash_mla_forward_reference(q, k, v, scale=None, out_dtype=None, return_lse=False):
    """
    Compute a reference Flash MLA forward output with stable softmax.
...
def flash_mla_backward_reference(q, k, v, o, grad_o, scale=None):
    """
    Compute a reference Flash MLA backward pass with stable softmax.
...
def benchmark_torch_flash_mla_backward(q, k, v, o, grad_o, warmup, rep):
    """
    Benchmark the reference Flash MLA backward implementation.
    ...
    def op():
        ...
        Main feature:
            Computes Flash MLA backward gradients.
        ...
        Outputs:
            None
        """
        _ = flash_mla_backward_reference(q, k, v, o, grad_o)

    return triton.testing.do_bench(op, warmup=warmup, rep=rep)


def benchmark_triton_flash_mla_forward(q, k, v, warmup, rep):
    """
    Benchmark the Triton Flash MLA forward kernel.
    ...
    def op():
        """
        Invoke the Triton Flash MLA forward kernel.

        Main feature:
            Dispatches the Triton Flash MLA forward kernel.
        ...
        Outputs:
            None
        """
        _ = launch_flash_mla_forward_bf16_kernel(q, k, v)
...
def bytes_accessed_forward(q, k, v, o, lse):
    """
    Estimate bytes moved by Flash MLA forward.
...
def flops_flash_mla_forward(batch, q_heads, seqlen_q, seqlen_kv, head_dim_qk, head_dim_v):
    """
    Estimate FLOPs for Flash MLA forward (QK + PV).
...
def bytes_accessed_backward(q, k, v, lse, grad_o, grad_q, grad_k, grad_v):
    """
    Estimate bytes moved by Flash MLA backward.
...
def flops_flash_mla_backward(batch, q_heads, seqlen_q, seqlen_kv, head_dim_qk, head_dim_v):
    """
    Estimate FLOPs for Flash MLA backward (QK + dP + dQ + dK + dV).
...
def generate_inputs(seq_len, head_dim_qk, head_dim_v, num_query_heads, num_kv_heads, device):
    """
    Generate Flash MLA backward inputs for testing.
    ...
    o = flash_mla_forward_reference(q, k, v, out_dtype=q.dtype)
...
def run_tests():
    """
    Validate Flash MLA forward and backward kernels.
    ...
    grad_q_ref, grad_k_ref, grad_v_ref = flash_mla_backward_reference(q, k, v, o, grad_o)

    out_triton, lse_triton = launch_flash_mla_forward_bf16_kernel(q, k, v)
    out_ref, lse_ref = flash_mla_forward_reference(q, k, v, out_dtype=q.dtype, return_lse=True)
    ...
    grad_q_triton, grad_k_triton, grad_v_triton = launch_flash_mla_backward_bf16_kernel(
        q,
        k,
        v,
        o,
    ...
    forward_ms = benchmark_triton_flash_mla_forward(
        q,
        k,
        v,
        warmup=warmup_iters,
    ...
    backward_flops = flops_flash_mla_backward(
        batch=1,
        q_heads=q.shape[0],
        seqlen_q=q.shape[1],
        seqlen_kv=k.shape[1],
